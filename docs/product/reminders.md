...
Use the dedicated elapsed break counter, never the Current work metric. A fresh counter may correct a saved break start earlier when an older app seeded it from the zero work counter. Ignore two-second/one-minute precision noise and never extend the deadline from a stalled counter. Correcting the start releases reservations only for stages whose old deadlines were still in the future, allowing one overdue catch-up without replaying already-due stages; the break's silence setting is preserved.
