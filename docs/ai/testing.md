...
Closed-panel fixtures verify hidden reads can reveal read-only history, visible reads leave the launcher alone, open panels are not toggled closed, focused/dirty inputs block navigation, and asynchronous rendering is debounced. Counter assertions verify no attendance action is clicked.
