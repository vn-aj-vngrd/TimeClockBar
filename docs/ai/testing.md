...
Closed-panel fixtures verify hidden reads can reveal read-only history through the labeled sidebar launcher or compact header timer chip, visible reads leave the launcher alone, open panels are not toggled closed, focused/dirty inputs block navigation, and asynchronous rendering is debounced. Counter assertions verify no attendance action is clicked. A floating-panel fixture places unrelated content in a sibling main element and verifies that both the break timer and recorded dates come from the clock panel.
