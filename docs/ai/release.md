...
4. For GitHub Releases, run the `Release` workflow in GitHub Actions after the release changes are on `main`. It computes the version from commits, creates the `v<version>` tag, and uploads the zip to the release.
