...
- The floating clock panel is a sibling of the page's main content; extraction uses the attendance control's nearest clock-panel container before falling back to page landmarks. The website unmounts history when its details panel closes. Hidden-page reads may open its exact Time Clock navigation launcher (or the compact header's structurally identified timer chip) after reload, with input protection, an already-open metrics check, and a 15-second debounce. Visible pages are left under user control; attendance and report buttons are never invoked.
- Attendance classification normally requires a visible, enabled attendance control. Sidebar timers alone cannot distinguish work from a break. Fully transparent or zero-size clipped action containers are excluded. Offscreen WebKit can stall the website's break-entry animation, leaving rendered button text empty: the enabled `end-break` ID plus the exact break notice inside a known clock panel provides a narrow fallback. This fallback reads the notice's own elapsed counter from DOM text, rejects explicitly hidden/disabled controls, and never invokes the button. Active work has a matching fallback requiring both enabled `take-break` and `clock-out` controls in the same panel, so returning from a break remains readable while offscreen.
