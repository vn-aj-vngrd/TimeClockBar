...
TodayDashboard also resolves File report completion from the selected shift's persisted clock-out checkpoint. The user confirms the website requires a filed report before clock-out. This inference survives refresh/restart, excludes pre-shift clocked-out status, and cannot carry over to another work date. Opening Report alone never completes it.
