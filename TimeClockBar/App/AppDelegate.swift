...
final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, NSPopoverDelegate, UNUserNotificationCenterDelegate {
    ...
    private func bindAppTheme() {
        appThemeCancellable = controller.$appTheme
            .receive(on: RunLoop.main)
            .sink { [weak self] theme in
                self?.applyAppTheme(theme)
            }
    }

    private func applyAppTheme(_ theme: TimeclockAppTheme) {
    ...
    private func showAboutWindow() {
        if let aboutWindow {
            NSApp.activate(ignoringOtherApps: true)
            aboutWindow.appearance = controller.appTheme.appearance
        ...
        window.appearance = controller.appTheme.appearance
        window.center()
